...
class Edge(nn.Module):
    def __init__(self, ops, dim, stride, op_size, name, data_index=0, prune=True, lineage=None):
        super().__init__()
    ...
    def deadhead(self, prune_interval):
        self.prev_num_ops = self.num_ops
        dhs = [op.name for i, op in enumerate(self.ops) if op.deadhead(prune_interval)]
        self.used = self.get_edge_size()
    ...
    def genotype_compression(self, soft_ops=0, hard_ops=0):
        for op in self.ops:
            ...
            soft_ops += op.pruner.sg() * op.pruner.mem_size if not op.zero else 0
    ...
    def get_growth(self):
        ...
        if len(self.growth_factor['weight']) == 0 or all_none_grad or all_none_weight:
            ...
            out = {
                ...
                'abs_std_weight': np.std(np.abs(self.growth_factor['weight']))/self.avg_path_length,
                'abs_std_grad': np.std(np.abs(self.growth_factor['grad']))/self.avg_path_length,
                'abs_mean_weight': np.mean(np.abs(self.growth_factor['weight']))/self.avg_path_length,
                'abs_mean_grad': np.mean(np.abs(self.growth_factor['grad']))/self.avg_path_length,
                'shap': self.shap
            }
            return {k:(v,self.avg_path_length) if v is not None else v for k,v in out.items()}
...
class Net(nn.Module):
    ...
    def deadhead(self, prune_interval):
        ...
        for i, cell in enumerate(self.cells):
            for key in cell.edges.keys():
                dhs = cell.edges[key].deadhead(prune_interval)
                deadheads += len(dhs)
                for dh in dhs:
    ...
    def genotype_compression(self, used_ratio=False):
        ...
        hard_ops = 0
        out = [None,None]
        if not used_ratio and self.prune:
            for cell in self.cells:
